...
impl<'a> Trailer {
    ...
    pub(crate) async fn get_witch_mathing_cargo(client: &Data<DbClient>, cargo_type: &str, pickup_date: &str, drop_date: &str, excluded: Vec<String>)
    ...
    Result<BTreeMap<String, Vec<Trailer>>, APIError> {
        ...
        for item in load {
            let already_selected = result.values()
                .flat_map(|trailers: &Vec<Trailer>|
                    trailers.iter()
                        .map(|trailer| format!("trailer:⟨{}⟩", trailer.plate)))
                .collect();
            let response = Trailer::get_witch_mathing_cargo(client, &item.cargo_name, pickup_date, drop_date, already_selected).await;
